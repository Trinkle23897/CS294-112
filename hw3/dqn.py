...
class QLearner(object):
	...
	def __init__(
		...
		######
		q = q_func(obs_t_float, self.num_actions, scope="q_func", reuse=False) # new
		q_tp1 = q_func(obs_tp1_float, self.num_actions, scope="target_q_func", reuse=False) # old
		self.best_action = tf.argmax(q, axis=1)
		if double_q:
			action_new = tf.argmax(q_func(obs_tp1_float, self.num_actions, scope="q_func", reuse=True), axis=1)
			target_q = self.rew_t_ph + (1 - self.done_mask_ph) * gamma * tf.stop_gradient(tf.reduce_sum(tf.one_hot(action_new, self.num_actions) * q_tp1, axis=1))
		else:
			target_q = self.rew_t_ph + (1 - self.done_mask_ph) * gamma * tf.stop_gradient(tf.reduce_max(q_tp1, axis=1))
		actual_q = tf.reduce_sum(tf.one_hot(self.act_t_ph, self.num_actions) * q, axis=1)
		self.total_error = tf.reduce_mean(huber_loss(target_q - actual_q))
		q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="q_func")
	...
	def step_env(self):
		...
		if not self.model_initialized or np.random.random() < self.exploration.value(self.t):
			...
			action = self.session.run(self.best_action, feed_dict={self.obs_t_ph: self.replay_buffer.encode_recent_observation()[None]})[0]
	...
	def update_model(self):
		...
		if (self.t > self.learning_starts and \
			...
			# YOUR CODE HERE
			obs, act, rew, nxtobs, done = self.replay_buffer.sample(self.batch_size)
			if not self.model_initialized:
				initialize_interdependent_variables(self.session, tf.global_variables(), {
					self.obs_t_ph: obs,
					self.obs_tp1_ph: nxtobs
				})
