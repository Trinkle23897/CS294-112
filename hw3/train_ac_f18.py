...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        vsp = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
        adv_n = re_n + self.gamma * (1 - terminal_n) * vsp - vs
        if self.normalize_advantages:
            adv_n = (adv_n - adv_n.mean(axis=0)) / (adv_n.std(axis=0) + 1e-7) # YOUR_HW2 CODE_HERE
        return adv_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_target_updates):
            tar_v = re_n + self.gamma * (1 - terminal_n) * self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
            for j in range(self.num_grad_steps_per_target_update):
                self.sess.run(self.critic_update_op, feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: tar_v})
