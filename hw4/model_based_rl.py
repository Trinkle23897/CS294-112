...
class ModelBasedRL(object):
    ...
    def _train_policy(self, dataset):
        ...
        for e in range(self._training_epochs):
            for s, a, s_, r, d in dataset.random_iterator(self._training_batch_size):
                losses.append(self._policy.train_step(s, a, s_))
    ...
    def run_q1(self):
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            ...
            pred_state = states[0]
            for a in actions:
                pred_state = self._policy.predict(pred_state, a)
